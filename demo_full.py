...
def start_wifi_connect(ssid, password):
    ...
    def on_done(ok, msg):
        ...
        wifi_connecting = False
        wifi_result_msg = "conectado" if ok else f"error: {msg[:30]}"

    connect_wifi(ssid, password, on_done)


def on_encoder(n, delta):
    global page, wifi_index, kbd_char_index, wled_preset_index
    if not delta:
        return
    if screen == "pages":
        if page == 0 or page == 1:
            ...
            if n == 1 and preset_list:
                wled_preset_index = (wled_preset_index + delta) % len(preset_list)
                preset_id, preset_name = preset_list[wled_preset_index]
                config["wled_preset"] = preset_id
                wled.apply_preset(preset_id)
                print(f"WLED: preset -> {preset_name}")
            elif n == 2:
        ...
        if n == 1:
            ...
            kbd_char_index = (kbd_char_index + delta * 10) % len(KEYBOARD_CHARS)
...
try:
    while True:
        ...
        if screen == "wifi_kbd" and key_press_ts[3] is not None and not key_long_fired[3]:
        ...
        with canvas(device) as draw:
            if screen == "pages":
                ...
                if page == 0 or page == 1:
                    ...
                    preset_list = sorted(wled.presets.items())
                    if preset_list:
                        idx = min(wled_preset_index, len(preset_list) - 1)
                        pid, pname = preset_list[idx]
                        draw.text((2, 11), f"{pname[:18]} [{idx+1}/{len(preset_list)}]", fill="white")
                    else:
                        draw.text((2, 11), "conectando WLED...", fill="white")
                ...
                if not wifi_networks:
                    ...
                    draw.text((2, 16), net["ssid"][:21], fill="white")
                    draw.text((2, 28), f"senal: {net['signal']}%  {sec}", fill="white")
                    draw.text((2, 40), f"{wifi_index + 1}/{len(wifi_networks)}", fill="white")
                    draw.text((2, 52), "K1:elegir K2:buscar K3:volver", fill="white")

            elif screen == "wifi_kbd":
                net = wifi_networks[wifi_index]
                draw.text((2, 1), f"WIFI: {net['ssid'][:16]}", fill="white")
                draw.text((2, 14), f"clave: {wifi_password[-19:]}", fill="white")
                ch = KEYBOARD_CHARS[kbd_char_index]
                draw.text((2, 28), f"caracter: [{ch}]", fill="white")
                draw.text((2, 42), "K1:anade K2:borra", fill="white")
                ratio = held_ratio(3, LONG_PRESS_K3)
                if ratio > 0:
                    draw.text((2, 52), "conectando (manten K3)", fill="white")
                else:
                    draw.text((2, 52), "K3:cancela / manten=conecta", fill="white")
                draw.rectangle((2, 60, 2 + int(120 * ratio), 63), fill="white")
